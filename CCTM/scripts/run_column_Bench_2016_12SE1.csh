...
while ($TODAYJ <= $STOP_DAY )  #>Compare dates in terms of YYYYJJJ
  ...
  set YYYYMMDD = `$DATE_COMMAND "${TODAYG}" +%Y%m%d` #> Convert YYYY-MM-DD to YYYYMMDD
  set YYYYMM = `$DATE_COMMAND "${TODAYG}" +%Y%m`     #> Convert YYYY-MM-DD to YYYYMM
  set YYMMDD = `$DATE_COMMAND "${TODAYG}" +%y%m%d`   #> Convert YYYY-MM-DD to YYMMDD
  set YYYYJJJ = $TODAYJ
...
# ===================================================================
  ...
  set TODAYG = `$DATE_COMMAND -v +1d "${TODAYG}" +%Y-%m-%d` #> Add a day for tomorrow
  set TODAYJ = `$DATE_COMMAND "${TODAYG}" +%Y%j` #> Convert YYYY-MM-DD to YYYYJJJ
...
foreach it ( `seq ${NDAYS}` )
    ...
    set day = `$DATE_COMMAND -v +1d "${day}" +%Y-%m-%d`
end
